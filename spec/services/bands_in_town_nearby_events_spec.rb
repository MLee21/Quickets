...
RSpec.describe BandsInTownService, type: :model do 
  ...
  it "returns information for nearby events" do 
    VCR.use_cassette("get_nearby_events") do 
      ...
      expect(nearby_events.first.name).to eq("First Responders")
      expect(nearby_events.first.event_venue).to eq("Toby Keith's I Love This Bar and Grill")
    end
  end
  ...
  it "returns information for a different city's events" do 
    VCR.use_cassette("get_los_angeles_events") do 
      ...
      nearby_events = service.nearby_artists_at_events(location)
      expect(nearby_events.first.name).to eq("Adam Lambert")
      expect(nearby_events.first.event_venue).to eq("The Original High")
    end
  end
  ...
  it "returns information for a different city's events with no state specified" do 
    VCR.use_cassette("get_portland_events") do 
      location = "Portland"
      nearby_events = service.nearby_artists_at_events(location)
      expect(nearby_events.first.name).to eq("Mary Flower")
      expect(nearby_events.first.event_venue).to eq("Grant House")
    end
  end
